...
from snorkel.models import Span, Candidate, Document, Sentence, TemporarySpan, GoldLabel, GoldLabelKey
...
class BratAnnotator(object):
    ...
    def __init__(self, session, candidate_class, encoding="utf-8",
                 annotator_name='gold'):#, address='localhost', port=8001):
        """
        ...
        :param port:
        """
        self.session = session # SnorkelSession
        ...
        self.brat_root = 'brat_imported_data'
    ...
    def map_annotations(self, session, annotation_dir, candidates, symmetric_relations=True):
        ...
        for doc_name in documents:
            ...
            for key in spans:
                #generated_id = doc_name + "::span::" + "XXX"
                brat_stable_ids_spans.append(spans[key].get_stable_id())
        ...
        mapped_cands, missed = [], []
        for span in brat_stable_ids_spans:
    ...
    def import_gold_labels(self, session, annotation_dir, candidates,
                           symmetric_relations=True,  annotator_name='gold'):
        """
        ...
        for c in mapped_cands:
            ...
            session.add(label)
        #print("SESSION SHOULD COMMIT NOW")
        session.commit()
    ...
    def _init_annotation_config(self, candidate_class, doc_root):
        ...
        shutil.copyfile("util/tools.conf".format(self.path),
...
class StandoffAnnotations(object):
    ...
    def __init__(self, tmpl_path='annotation.config.tmpl', encoding="utf-8"):
        ...
        #print(mod_path)
        #print("".join(open(mod_path, "rU").readlines()))
        mod_path = os.path.join("util", tmpl_path)
